...
for OutLoop=1:StaNum
	...
	[F,G,H]=GetConSis( vtC(:,k), posC(:,k), q_1, fn(:,k), Tg, Ta);
	
	%disp('INS size Pk')
	%disp(size(Pk))
	if mod(k,10)==0
		[ E_att, E_pos, E_vn, Xk, Pk ] = kalman_GPS_INS_correct(Xk,Qk,Pk, F, G, H ,ts,p_gps(:,k/10)-posC(:,k), Rk);
	else 
		[ E_att, E_pos, E_vn, Xk, Pk ] = kalman_GPS_INS_correct(Xk,Qk,Pk, F, G, H ,ts,0, Rk);
	end
